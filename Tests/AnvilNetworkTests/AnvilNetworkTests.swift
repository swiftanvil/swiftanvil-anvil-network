...
struct HTTPRequestTests {
    ...
    func emptyBodyDefault() throws {
        let request = try HTTPRequest(method: .get, url: #require(URL(string: "https://example.com")))
        #expect(request.body == .empty)
...
actor MockTransport: HTTPTransport {
    ...
    func enqueueRetryable(statusCode _: Int, retryAfter: String? = nil) {
...
struct HTTPClientTests {
    ...
    func getRequestWithMock() async throws {
        ...
        let request = try HTTPRequest(method: .get, url: #require(URL(string: "https://api.example.com/users/1")))
    ...
    func queryParameters() async throws {
        let mock = MockTransport()
        let request = try HTTPRequest(method: .get, url: #require(URL(string: "https://api.example.com/search")))
        await mock.enqueue(HTTPResponse(request: request, statusCode: 200, headers: HTTPHeaders(), body: Data()))

        let client = HTTPClient(transport: mock)
        _ = try await client.get("https://api.example.com/search")
            .query("q", "swift")
            .query("limit", "10")
            .send()
    ...
    func requestInterceptor() async throws {
        let mock = MockTransport()
        let request = try HTTPRequest(method: .get, url: #require(URL(string: "https://api.example.com")))
        await mock.enqueue(HTTPResponse(request: request, statusCode: 200, headers: HTTPHeaders(), body: Data()))

        let client = HTTPClient(transport: mock)
        let intercepted = await client.addingRequestInterceptor(TestAuthInterceptor(token: "secret"))
        _ = try await intercepted.get("https://api.example.com").send()
    ...
    func cacheAuthIsolation() async throws {
        let mock = MockTransport()
        let url = try #require(URL(string: "https://api.example.com/user"))
        let response = HTTPResponse(
    ...
    func loggerCapturesNetworkActivity() async throws {
        ...
        _ = try await client.get("https://api.example.com/users/1").decode(as: TestUser.self)
...
struct CacheTests {
    ...
    func lruEviction() async throws {
        ...
        await cache.set(
            HTTPResponse(
                request: HTTPRequest(method: .get, url: url1),
                statusCode: 200,
                headers: HTTPHeaders(),
                body: Data()
            ),
            for: HTTPRequest(method: .get, url: url1)
        )
        await cache.set(
            HTTPResponse(
                request: HTTPRequest(method: .get, url: url2),
                statusCode: 200,
                headers: HTTPHeaders(),
                body: Data()
            ),
            for: HTTPRequest(method: .get, url: url2)
        )
        await cache.set(
            HTTPResponse(
                request: HTTPRequest(method: .get, url: url3),
                statusCode: 200,
                headers: HTTPHeaders(),
                body: Data()
            ),
            for: HTTPRequest(method: .get, url: url3)
        )
